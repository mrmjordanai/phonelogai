...
export class PIIDetector {
  ...
  private contextPatterns: Record<PIIType, RegExp[]>
  private replacements: Record<PIIType, string>
  ...
  constructor(config: Partial<PIIDetectionConfig> = {}) {
    this.config = {
      ...
      batchSize: 100,
      customPatterns: {},
      customReplacements: {} as Partial<Record<PIIType, string>>,
      ...config
    }
   ...
   */
  async detectAndAnonymize(text: string): Promise<PIIDetectionResult> {
   ...
   */
  private async performRegexDetection(text: string): Promise<PIIMatch[]> {
    const matches: PIIMatch[] = []
   ...
   */
  private async performMLDetection(text: string): Promise<PIIMatch[]> {
   ...
   */
  private async detectNamesWithML(text: string): Promise<PIIMatch[]> {
    const matches: PIIMatch[] = []
   ...
   */
  private initializePatterns(): Record<PIIType, RegExp[]> {
    return {
      phone_number: [
        // US/Canada formats
   ...
   */
  private initializeContextPatterns(): Record<PIIType, RegExp[]> {
    return {
      phone_number: [
   ...
   */
  private initializeReplacements(): Record<PIIType, string> {
    return {
      person_name: '[NAME]',
      phone_number: '[PHONE]',
      email_address: '[EMAIL]',
      ...
      custom: '[REDACTED]',
      ...this.config.customReplacements
    }
  }
   ...
   */
  private calculatePatternConfidence(piiType: PIIType, match: string, context: string): number {
   ...
   */
  private calculateNameMLConfidence(name: string, context: string): number {
   ...
   */
  private filterAndSortMatches(matches: PIIMatch[]): PIIMatch[] {
   ...
   */
  private generateAnonymizedText(text: string, matches: PIIMatch[]): string {
   ...
   * Calculate overall confidence score
   */
  private calculateOverallConfidence(matches: PIIMatch[]): number {
